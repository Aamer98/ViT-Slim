...
class BaseModel(nn.Module):
    ...
    def get_sparsity_loss(self, device):
        ...
        if 'none' in self.prune_layers:
            return torch.FloatTensor([0.0]).to(device), torch.FloatTensor([0.0]).to(device), torch.FloatTensor([0.0]).to(device)
        
        elif 'patch' not in self.prune_layers:
            loss_attn = torch.FloatTensor([]).to(device)
            loss_mlp = torch.FloatTensor([]).to(device)
            for l_block in self.searchable_modules:
                if hasattr(l_block, 'num_heads'):
                    zeta_attn, _ = l_block.get_zeta()
                    loss_attn = torch.cat([loss_attn, torch.abs(zeta_attn.view(-1))])
                else:
                    loss_mlp = torch.cat([loss_mlp, torch.abs(l_block.get_zeta().view(-1))])
            return torch.sum(loss_attn).to(device), torch.sum(loss_mlp).to(device), torch.FloatTensor([0.0]).to(device)
