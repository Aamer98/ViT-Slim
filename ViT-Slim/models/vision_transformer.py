...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.attn = ModuleInjection.make_searchable_attn(self.attn, prune_layers, head_search, uniform_search)
        # NOTE: drop path for stochastic depth, we shall see if this is better than dropout here
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.norm2 = norm_layer(dim)
        mlp_hidden_dim = int(dim * mlp_ratio)
        self.mlp = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)
        self.mlp = ModuleInjection.make_searchable_mlp(self.mlp, prune_layers)
...
class VisionTransformer(BaseModel):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 act_layer=None, weight_init='', head_search=False, uniform_search=False, prune_layers = ['node']):
        """
        Args:
        ...
        self.blocks = nn.Sequential(*[
            Block(
                ...
                head_search=head_search, uniform_search=uniform_search, prune_layers=prune_layers)
