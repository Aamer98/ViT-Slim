...
def profile_evaluation(model, data_loader, device, max_steps=5, log_dir="./log"):
    """
    Profiles the model for a few evaluation steps, measuring latency and FLOPs per layer/operator.
    
    Args:
        model (nn.Module): The ViT model to profile.
        data_loader (DataLoader): A DataLoader for the evaluation dataset.
        device (torch.device): The device (CPU/CUDA) to run on.
        max_steps (int): Number of batches to profile (to avoid large trace files).
        log_dir (str): Directory where profiler traces will be written (for TensorBoard).
    """
    ...
    with torch.profiler.profile(
        ...
        # Run a few steps of evaluation under the profiler
        for step, (images, targets) in enumerate(data_loader):
            ...
            # Notify profiler that we’ve finished a profile step
            prof.step()

    # Print an aggregated summary of events
    # You can sort by "cpu_time_total", "cuda_time_total", "self_cuda_time_total", "flops", etc.
